...
public class MovieRatingsRepository : IMovieRatingsRepository
{
    ...
    {
        return await _connection.QueryAsync<int>("SELECT rating FROM ratings WHERE movie_id=@movieId", new { movieId });
    }

    public Task AddRatingAsync(int movieId, int ratingNumber)
    {
        string sql = @"INSERT INTO ratings (movie_id, rating) VALUES (@movieId, @ratingNumber)";
